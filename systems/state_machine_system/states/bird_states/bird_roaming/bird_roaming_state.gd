extends State

@export var parent: Node2D
@export var animated_sprite_2d: AnimatedSprite2D
@export var area_2d: Area2D
@export var action_delay_range: Vector2 = Vector2(0.5, 2.5)

@onready var roaming_action_timer: Timer = $RoamingActionTimer

var _is_roaming_action: bool = false

## -- overrides
func _on_enter() -> void:
	area_2d.area_entered.connect(_on_area_2d_area_entered)
	animated_sprite_2d.play("idle_" + parent.direction_name)

func _on_exit() -> void:
	area_2d.area_entered.disconnect(_on_area_2d_area_entered)
	animated_sprite_2d.stop()

func _perform_action() -> void:
	var rng: float = randf_range(0,1)
	if rng > 0.5:
		transition.emit("roamwalking")
		_is_roaming_action = false
	else:
		transition.emit("emoting")
		_is_roaming_action = false

func _fly_away() -> void:
	_is_roaming_action = false
	transition.emit("takeoff")

func _on_physics_process(_delta: float) -> void:
	if _is_roaming_action:
		if parent.total_actions < parent.max_actions:
			_perform_action()
		else:
			_fly_away()
	else:
		if roaming_action_timer.time_left > 0.0: return # timer already started
		animated_sprite_2d.play("idle_" + parent.direction_name)
		var rand_delay: float = randf_range(action_delay_range.x, action_delay_range.y)
		roaming_action_timer.start(rand_delay)

func _on_area_2d_area_entered(area: Area2D) -> void:
	if area is Hitbox: # player or npc
		transition.emit("takeoff")

func _on_roaming_action_timer_timeout() -> void:
	_is_roaming_action = true
