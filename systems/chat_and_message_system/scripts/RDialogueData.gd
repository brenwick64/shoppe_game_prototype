...
@export var statements: Array[RMessageData]

func get_conversation_starter(message_context: RMessageContext) -> RMessageData:
...
func get_statement(message_context: RMessageContext) -> RMessageData:
	var filtered_statements: Array[RMessageData] = statements.filter(
		func(stmt: RMessageData): return Utils.arrays_overlap(message_context.subject_tags, stmt.context.subject_tags)
	)
	# TODO: rank/score for relevance
	return filtered_statements.pick_random() if filtered_statements else null
...
func _get_correct_reply_type(type: Constants.CHAT_MESSAGE_TYPE) -> Constants.CHAT_MESSAGE_TYPE:
	match type:
		...
		#TODO: more cases
		_: return Constants.CHAT_MESSAGE_TYPE.STATEMENT
