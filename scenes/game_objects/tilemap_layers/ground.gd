...
func _ready() -> void:
	tile_manager = get_parent().tile_manager

func _use_tile_data_runtime_update(coords: Vector2i) -> bool:
	var tile_gp: Vector2 = tile_manager.get_global_pos_from_tile(self, coords)
	for layer: TileMapLayer in intersecting_layers:
		var layer_coords: Vector2i = tile_manager.get_tile_from_global_pos(layer, tile_gp)
